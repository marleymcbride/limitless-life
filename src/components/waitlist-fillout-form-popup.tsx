...
interface WaitlistFilloutFormPopupProps {
  onClose?: () => void;
}

export default function WaitlistFilloutFormPopup({ onClose }: WaitlistFilloutFormPopupProps) {
  ...
  const handleClose = () => {
    if (confirm('Close this form and return to the waitlist page?')) {
      setIsAnimatingOut(true);
      setTimeout(() => {
        router.replace('/waitlist');
      }, 300);
    ...
    return (
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm transition-opacity duration-300">
        <div className="bg-white rounded-lg shadow-2xl p-8 max-w-md mx-4 text-center">
          ...
          <p className="text-gray-600 text-sm">Redirecting to waitlist page...</p>
        </div>
      </div>
    );
  }
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto">
              <FilloutStandardEmbed
                ...
                onSubmit={() => {
                  console.log('[WaitlistFillout] Form submitted');
                  handleFormSuccess();
                }}
                onReady={() => {
                  console.log('[WaitlistFillout] Form ready');
                  console.log('[WaitlistFillout] Pre-filling with:', { name, email });
                  setTimeout(() => {
                    setIsLoading(false);
                  }, 400);
                }}
