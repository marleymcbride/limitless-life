...
export default function PricingSelector({ showEnroll: externalShowEnroll = false, onClose, initialTier }: PricingSelectorProps) {
  ...
  // Use external showEnroll if provided, otherwise manage internal state
  const [internalShowEnroll, setInternalShowEnroll] = useState(false);

  // Show enroll modal when either external prop is true or internal state is true
  useEffect(() => {
    if (externalShowEnroll) {
      setInternalShowEnroll(true);
    }
  }, [externalShowEnroll]);

  // Pre-select tier if initialTier is provided
  useEffect(() => {
    if (initialTier && (initialTier === 'protocol' || initialTier === 'life' || initialTier === 'life-whatsapp' || initialTier === 'concierge')) {
      setSelectedTier(initialTier as Tier);
    }
  }, [initialTier]);

  // Block scrolling when enroll modal is open
  useEffect(() => {
