...
export default function WaitlistDepositCTA() {
  ...
  const handleDeposit = async (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    try {
      console.log('[Main Waitlist Deposit] Creating Stripe checkout session');
      ...
      console.log('[Main Waitlist Deposit] Checkout session created:', data.sessionId);

      // Immediately redirect to Stripe Checkout
      if (data.url) {
        console.log('[Main Waitlist Deposit] Redirecting to Stripe...');
        window.location.href = data.url;
      } else {
        throw new Error('No checkout URL returned');
      }
    } catch (err) {
      console.error('[Main Waitlist Deposit] Error:', err);
