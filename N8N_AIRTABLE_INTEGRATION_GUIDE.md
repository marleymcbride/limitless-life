...
### Data Source: Railway PostgreSQL
...
│  • users (id, email, lead_score, lead_temperature, etc.)   │
│  • events (id, user_id, event_type, event_data, etc.)     │
│  • payments (id, user_id, stripe_payment_intent_id, etc.) │
│  • lead_alerts (id, user_id, alert_type, sent_at, etc.)   │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────────┐
│                   n8n Webhook Layer (Sync)                   │
...
type EventType =
  ...
  | 'tier_click'          // Clicked tier button (15pts)
  | 'tier_view'           // Viewed tier options (15pts)
  | 'tier_select_protocol'     // Selected Protocol (10pts)
  | 'tier_select_life'        // Selected Life (15pts)
  | 'tier_select_whatsapp'    // Selected Life+WhatsApp (20pts)
  | 'tier_select_concierge'   // Selected Concierge (25pts)
  // Phase 2: Purchase Intent (60-100pts) - milestone overrides
  | 'payment_plan_select'       // Selected payment plan → 85pts
  | 'stripe_checkout_initiated' // Initiated Stripe → 95pts
  | 'payment_complete'          // Completed purchase → 100pts
  | 'scroll_depth'       // Scroll tracking (0pts)
  | 'vsl_complete';       // VSL complete (60pts, but counted via milestone)
```
...
- `Elite` = Health Concierge tier ($6,897)
...
```json
{
  ...
  "tier": "Premium",
  "amount": 439700,
  "stripePaymentId": "pi_3N5xQ9oL1fQ7x9xQ7x9",
...
const ENGAGEMENT_POINTS = {
  vsl_start: 5,
  vsl_milestone: 0,  // Calculated as (percent / 100) * 60
  email_submit: 10,
...
cta_click                         | 1     | +5     | 10          | cold
email_submit                       | 1     | +10    | 20          | cold
application_start                   | 1     | +30    | 50          | warm
tier_view (pricing modal opens)    | 1     | +15    | 65          | warm
...
**Key File:** `src/app/api/webhooks/stripe/route.ts` (lines 208-224)
...
5. Check n8n executions: https://n8n.marleymcbride.co → Workflows → Executions
...
- PostgreSQL is source of truth, can re-sync if needed
...
4. Rate limits (Airtable free tier: 5 requests/second)
